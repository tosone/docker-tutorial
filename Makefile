...
install-docker-compose:
	sudo curl -L https://github.com/docker/compose/releases/download/$(COMPOSE_VERSION/ocker-compose-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -m` -o /usr/bin/docker-compose
...
.PHONY: $(all) $(others)
$(all) $(others):
	docker-compose up -p ${PROJECT_NAME} --force-recreate -d $@

.PHONY: shadowsocks-proxy
shadowsocks-proxy:
	cd $@ && sh gen.sh > modules-enabled/shadowsocks.conf
	docker-compose up -p ${PROJECT_NAME} --force-recreate -d --build $@
...
etcd:
	...
	cd $@ && docker-compose up -p ${PROJECT_NAME} --force-recreate -d
...
.PHONY: stop
stop:
	docker-compose stop $(all) $(others) traefik shadowsocks-proxy postgres
